...
// 🔹 NOVO: Interface para o estado local do frontend (para corresponder à Tabela)
interface AppBusSchedule {
    id: number;
    route: string;  // Mapeado de 'origem' do backend
    schedule: string; // Mapeado de 'horario' do backend
}

// 🔹 NOVO: Interface para a resposta do novo backend
...
export default function AdminDashboardPage() {
    ...
    // 🔹 ATUALIZADO: Estado agora usa a interface local AppBusSchedule
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // 🔹 REMOVIDO: Chamada antiga de axios para /api/routes.
            // A função fetchSchedules() (chamada abaixo) agora cuida disso.
        };
    ...
    // 🔹 REMOVIDO: Estados de edição não são mais suportados pelo novo controller
    // const [editingRoute, setEditingRoute] = useState<any>(null);
    // const [editRoute, setEditRoute] = useState('');
    // const [editTime, setEditTime] = useState('');
    // const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);

    // 🔹 Função para buscar horários (ATUALIZADA)
    const fetchSchedules = () => {
        // 🔹 ATUALIZADO: URL do endpoint
        fetch("http://localhost:8080/horarios-onibus")
            ...
            .then(data => {
                ...
                // 🔹 ATUALIZADO: Mapeamento da resposta do backend (origem, horario)
                // para o estado do frontend (route, schedule)
    ...
    // 🔹 Adicionar horário no banco (ATUALIZADO)
        ...
        if (newRoute && newTime) {
            try {
                ...
                const response = await fetch("http://localhost:8080/horarios-onibus", {
                    ...
                    // 🔹 ATUALIZADO: Corpo da requisição para bater com CriarHorarioRequest
                ...
                if (response.ok) {
                    ...
                    if (contentType && contentType.includes("application/json")) {
                        // 🔹 ATUALIZADO: Tipagem da resposta
                        const saved: BackendScheduleResponse = await response.json();

                        // 🔹 ATUALIZADO: Mapeamento da resposta para o estado
    ...
    // 🔹 Remover horário do banco (ATUALIZADO)
    const handleRemoveSchedule = async (id: number) => {
        try {
            // 🔹 ATUALIZADO: URL do endpoint e ID na URL
            const response = await fetch(`http://localhost:8080/horarios-onibus/${id}`, {
                ...
                // 🔹 REMOVIDO: O corpo (body) não é mais necessário
            });
    ...
    // 🔹 REMOVIDO: Função handleDeleteAllSchedules
    // O novo controller não possui um endpoint /delete-all

    // 🔹 REMOVIDO: Função handleOpenEditDialog
    // O novo controller não possui um endpoint de atualização (PUT)

    // 🔹 REMOVIDO: Função handleSaveEdit
    // O novo controller não possui um endpoint de atualização (PUT)

    // ... (Restante das funções handleDismissReport, handleSendNotification, etc. permanecem iguais)
    const handleDismissReport = (id: number) => {
        // A lógica original usava 'string' para id, mas o tipo UserReport usa 'number'. Ajustado para 'number'.
        ...
        <div className="flex flex-col min-h-screen bg-background">
            ...
            <main className="flex-grow container mx-auto px-4 md:px-6 py-8">
                ...
                <div className="grid gap-8 md:grid-cols-1 lg:grid-cols-2">
                    ...
                    {/* ... (Restante do JSX para Notificações, Denúncias e Usuários permanece igual) ... */}
